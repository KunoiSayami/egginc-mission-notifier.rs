...
use crate::{
    bot::replace_all,
    egg::types::ContractGradeSpec,
    types::{BASE64, fmt_time_delta, return_tf_emoji, timestamp_to_string},
};

pub static DEFAULT_NICKNAME: LazyLock<String> = LazyLock::new(|| "N/A".into());
...
impl SubscribeInfo {
    pub fn check_user(&self, user: i64) -> bool {
        self.users.iter().any(|x| x == &user)
    }

    pub fn add_user(&mut self, user: i64) -> bool {
    ...
    pub fn random(users: Vec<i64>, est: i64) -> Self {
        let gen_faked = || format!("Faked-{}", Alphanumeric.sample_string(&mut rand::rng(), 16));
        Self {
            est,
            id: gen_faked(),
            room: gen_faked(),
...
impl FromRow<'_, SqliteRow> for SubscribeInfo {
    fn from_row(row: &SqliteRow) -> sqlx::Result<Self> {
        Ok(Self {
            id: row.try_get("id")?,
...
impl PartialEq for SubscribeInfo {
    fn eq(&self, other: &Self) -> bool {
        self.id == other.id && self.room == other.room
    }
}

impl Hash for SubscribeInfo {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        self.id.hash(state);
...
pub fn convert_set<T: Eq + Hash>(v: Vec<HashSet<T>>) -> Vec<T> {
    v.into_iter()
        .reduce(|mut acc, x| {
            acc.extend(x);
            acc
